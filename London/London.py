...
def distances(start_address,end_address):
	...
	start_lat, start_longt = gmaps.address_to_latlng(start_address+', London, UK')	#Getting the longtitudes and lattitudes of given addresses
	end_lat, end_longt = gmaps.address_to_latlng(end_address+', London, UK')
	sensor='sensor=false'
...
fout_name_base, fout_name = 'london_results','london_results.txt'
ver = 1
...
for i,v in enumerate(a):
	...
	if ratio == False : 
		print 'UK postcodes are retarded'
		time.sleep(1)
		continue
	ratios.append(ratio)
	outf.write(v+',')
